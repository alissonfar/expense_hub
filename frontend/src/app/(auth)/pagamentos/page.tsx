...
export default function PagamentosPage() {
  ...
  // Buscar pagamentos usando o novo hook
  const { data: pagamentosResponse, isLoading } = usePagamentos({
    search: searchTerm || undefined,
  });
  
  // Extrair dados dos pagamentos
  const pagamentos = pagamentosResponse?.data?.pagamentos || [];
  
  // Buscar todos os pagamentos (não filtrados) para totais gerais
  const { data: todosPagamentosResponse } = usePagamentos({
    limit: 1000,
  });
  
  // Extrair dados dos pagamentos para totais
  const todosPagamentos = todosPagamentosResponse?.data?.pagamentos || [];
      ...
      cell: ({ row }: { row: Row<Pagamento> }) => (
        <div className="flex items-center gap-2">
          <Button 
            variant="ghost" 
            size="icon"
            onClick={() => router.push(`/pagamentos/${row.original.id}`)}
